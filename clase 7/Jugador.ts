...
import { _decorator, Component, Node, systemEvent, SystemEvent, EventKeyboard, Animation, Camera, Collider, ICollisionEvent, AudioSource, Label } from 'cc';
...
export class Jugador extends Component {
    ...
    @property(Label)
    private labelEnemigo1: Label;

    @property(Label)
    private labelEnemigo2: Label;
    ...
    update() {
        if (PublicData.vidaEnemigo === 0 && PublicData.vidaEnemigo2 === 0) {
    ...
    collisionEnter(event: ICollisionEvent) {
        if (this._atacando) {
            ...
            if (event.otherCollider.node.name == "NodoEnemigoCocos") {
                console.log(PublicData.vidaEnemigo);
                PublicData.vidaEnemigo = PublicData.vidaEnemigo - 1;
                console.log(PublicData.vidaEnemigo);
                this.labelEnemigo1.string = "Vida enemigo 1: " + PublicData.vidaEnemigo;
            }
            else {
                console.log(PublicData.vidaEnemigo2);
                PublicData.vidaEnemigo2 = PublicData.vidaEnemigo2 - 1;
                console.log(PublicData.vidaEnemigo2);
                this.labelEnemigo2.string = "Vida enemigo 2: " + PublicData.vidaEnemigo2;
            }
            this.getComponent(AudioSource).play();

            // PublicData.vidaEnemigo -= 1;
        }
    }

    restart() {
        this.gameOver.active = false;
        PublicData.vidaEnemigo = 3;
        PublicData.vidaEnemigo2 = 3;
        this._dirActual = DIRS.IDLE;
        this._animActual = ANIMS.IDLE;
        this._animacion.play("cocos_anim_idle");
        this.labelEnemigo2.string = "Vida enemigo 2: " + PublicData.vidaEnemigo2;
        this.labelEnemigo1.string = "Vida enemigo 1: " + PublicData.vidaEnemigo;
        console.log("restart jugador");
        this.node.setPosition(0, 2, 0);
        this.node.setRotationFromEuler(0, 0, 0);
    }
    onDestroy() {
